...
Author Homebase adalah rumah digital lengkap seorang penulis KBM: identitas, katalog, **sistem pengumpulan database pembaca**, jalur kabar rilis, peta baca, etalase kolaborasi, dan—bila berlangganan—**MarkasAuthor Assistant** sebagai remote control operasional melalui Telegram. Ini bukan "website profil" — ini sistem dengan **satu loop inti** yang harus bekerja:
...
- Tidak menyediakan CMS web bebas atau akses edit source code. Self-service pada tier hero dibatasi pada command Telegram yang diizinkan sistem.
...
### 3.3 MarkasAuthor Assistant ⭐ (fitur recurring — Telegram untuk operasional penulis)
**Kebutuhan:** setelah menerbitkan chapter di KBM, penulis dapat memperbarui website dan menyiapkan kabar rilis dari chat, tanpa login dashboard atau meminta developer mengedit manual.

**Posisi produk:** Telegram adalah back office/remote control untuk penulis. WhatsApp tetap front office/distribusi untuk pembaca. Fitur ini adalah add-on bulanan, bukan syarat agar situs publik tetap bisa dibuka.
...
```text
Penulis → Telegram Bot API → webhook terverifikasi → identitas & tenant
...
| CMS web bebas / edit source code | Tidak untuk hero; Telegram hanya menjalankan command yang diizinkan |
| Telegram Mini App/dashboard | Roadmap, bukan MVP (lihat §10) |
